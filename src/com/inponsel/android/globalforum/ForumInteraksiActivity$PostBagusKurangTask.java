...
public class this._cls0 extends AsyncTask
{
    ...
    protected transient Void doInBackground(Void avoid[])
    {
        try
        {
            if (android.os.y.PostBagusKurangTask >= 11)
            {
                avoid = StrictMode.getThreadPolicy();
                StrictMode.setThreadPolicy((new android.os.y.PostBagusKurangTask(avoid)).rites().rites());
...
_L1:
        Log.e("mArrayListData", String.valueOf(ForumInteraksiActivity.access$1(ForumInteraksiActivity.this).size()));
        Log.e("index_komposlike", idkom_pos);
        updateViewLikeDis(idkom_pos);
        if (android.os.y.idkom_pos >= 11)
        {
            (new <init>(ForumInteraksiActivity.this)).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new Void[0]);
            return;
        }
        ...
        catch (Void void1)
        {
          ...
          goto _L1
        (new <init>(ForumInteraksiActivity.this)).execute(new Void[0]);
        return;
        if (statuslike.equals("1"))
        {
