    ...
    protected transient Void doInBackground(Void avoid[])
    {
        try
        {
            if (android.os.y.PostBagusKurangTask >= 11)
            {
                ...
                StrictMode.setThreadPolicy((new android.os.y.PostBagusKurangTask(avoid)).skWrites().skWrites());
...
_L5:
        Log.e("mArrayListData", String.valueOf(ForumGlobalActivity.access$0(ForumGlobalActivity.this).size()));
        Log.e("index_komposlike", idkom_pos);
        updateViewLikeDis(idkom_pos);
        if (android.os.y.idkom_pos >= 11)
        {
            (new k(ForumGlobalActivity.this)).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new Void[0]);
            return;
        }
...
label0:
        ...
        notificationLikeHelper.completed("Artikel Ponsel", notificationLikeHelper.dislikefrontKomen);
          goto _L5
        (new k(ForumGlobalActivity.this)).execute(new Void[0]);
        return;
