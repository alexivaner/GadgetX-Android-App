    ...
    protected transient Void doInBackground(Void avoid[])
    {
        try
        {
            if (android.os.ty.PostBagusKurangTask >= 11)
            {
                ...
                StrictMode.setThreadPolicy((new android.os.ty.PostBagusKurangTask(avoid)).kWrites().kWrites());
...
_L5:
        Log.e("mArrayListData", String.valueOf(ConversationActivity.access$0(ConversationActivity.this).size()));
        Log.e("index_komposlike", idkom_pos);
        updateViewLikeDis(idkom_pos);
        if (android.os.ty.idkom_pos >= 11)
        {
            (new k(ConversationActivity.this)).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new Void[0]);
            return;
        }
...
label0:
        ...
        notificationLikeHelper.completed("Conversation", notificationLikeHelper.dislikefrontKomen);
          goto _L5
        (new k(ConversationActivity.this)).execute(new Void[0]);
        return;
