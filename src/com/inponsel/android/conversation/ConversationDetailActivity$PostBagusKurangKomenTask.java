...
public class this._cls0 extends AsyncTask
{
    ...
    protected transient Void doInBackground(Void avoid[])
    {
        try
        {
            if (android.os.stBagusKurangKomenTask >= 11)
            {
                avoid = StrictMode.getThreadPolicy();
                StrictMode.setThreadPolicy((new android.os.stBagusKurangKomenTask(avoid)).stBagusKurangKomenTask().stBagusKurangKomenTask());
...
_L1:
        Log.e("mArrayListData", String.valueOf(ConversationDetailActivity.access$2(ConversationDetailActivity.this).size()));
        Log.e("index_komposlike", idkom_pos);
        updateViewLikeDis(idkom_pos);
        if (android.os.kom_pos >= 11)
        {
            (new >(ConversationDetailActivity.this)).teOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new Void[0]);
            return;
        }
        ...
        catch (Void void1)
        {
          ...
          goto _L1
        (new >(ConversationDetailActivity.this)).te(new Void[0]);
        return;
        if (statuslike.equals("1"))
        {
