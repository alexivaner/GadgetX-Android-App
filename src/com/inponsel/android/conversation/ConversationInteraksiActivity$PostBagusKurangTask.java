    ...
    protected transient Void doInBackground(Void avoid[])
    {
        try
        {
            if (android.os.ty.PostBagusKurangTask >= 11)
            {
                ...
                StrictMode.setThreadPolicy((new android.os.ty.PostBagusKurangTask(avoid)).PostBagusKurangTask().PostBagusKurangTask());
...
_L1:
        Log.e("mArrayListData", String.valueOf(ConversationInteraksiActivity.access$1(ConversationInteraksiActivity.this).size()));
        Log.e("index_komposlike", idkom_pos);
        updateViewLikeDis(idkom_pos);
        if (android.os.ty.idkom_pos >= 11)
        {
            (new <init>(ConversationInteraksiActivity.this)).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new Void[0]);
            return;
        }
        ...
        catch (Void void1)
        {
          ...
          goto _L1
        (new <init>(ConversationInteraksiActivity.this)).execute(new Void[0]);
        return;
        if (statuslike.equals("1"))
        {
