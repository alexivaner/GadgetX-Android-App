...
public class this._cls0 extends AsyncTask
{
    ...
    protected transient Void doInBackground(Void avoid[])
    {
        try
        {
            if (android.os.tBagusKurangTask >= 11)
            {
                avoid = StrictMode.getThreadPolicy();
                StrictMode.setThreadPolicy((new android.os.tBagusKurangTask(avoid)).kWrites().kWrites());
...
_L1:
        Log.e("mArrayListData", String.valueOf(RSSInteraksiActivity.access$1(RSSInteraksiActivity.this).size()));
        Log.e("index_komposlike", idkom_pos);
        updateViewLikeDis(idkom_pos);
        if (android.os.om_pos >= 11)
        {
            (new <init>(RSSInteraksiActivity.this)).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new Void[0]);
            return;
        }
        ...
        catch (Void void1)
        {
          ...
          goto _L1
        (new <init>(RSSInteraksiActivity.this)).execute(new Void[0]);
        return;
        if (statuslike.equals("1"))
        {
