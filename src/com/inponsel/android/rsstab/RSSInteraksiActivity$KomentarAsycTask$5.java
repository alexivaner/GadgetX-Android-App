...
class val.tl_id
    implements android.view.ntarAsycTask._cls5
{
    ...
    public void onClick(View view)
    {
        if (cess._mth2(this._cls1.this).userFunctions.isUserLoggedIn(cess._mth2(this._cls1.this)))
        {
            ...
            if (android.os.erylike >= 11)
            {
                (new (cess._mth2(this._cls1.this))).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new Void[0]);
                return;
            } else
            {
                (new (cess._mth2(this._cls1.this))).execute(new Void[0]);
                return;
            }
        } else
        {
            view = new android.app.tBagusKurangTask.execute(cess._mth2(this._cls1.this));
            view.entarAsycTask("Untuk memberi penilaian harus login terlebih dahulu.");
            view.entarAsycTask("Tutup", new android.content.DialogInterface.OnClickListener() {
