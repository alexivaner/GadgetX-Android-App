...
public class this._cls0 extends AsyncTask
{
    ...
    protected transient Void doInBackground(Void avoid[])
    {
        try
        {
            if (android.os.omen.PostBagusKurangTask >= 11)
            {
...
_L1:
        Log.e("mArrayListData", String.valueOf(Hal2TLKomen.access$1(Hal2TLKomen.this).size()));
        Log.e("index_komposlike", idkom_pos);
        updateViewLikeDis(idkom_pos);
        if (android.os.omen.idkom_pos >= 11)
        {
            (new <init>(Hal2TLKomen.this)).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new Void[0]);
            return;
        }
        ...
        catch (Void void1)
        {
          ...
          goto _L1
        (new <init>(Hal2TLKomen.this)).execute(new Void[0]);
        return;
        if (statuslike.equals("1"))
        {
